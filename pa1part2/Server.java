		...
		try {
            ...
            for (int i = 0; i < maxUsersCount; i++){
            	if (threads[i] != null && threads[i] != this){
            		threads[i].output_stream.println("#newUser "+ name.split(" ")[1]);
            	}
            }
			...
			while (true) {
				...
				for (int i = 0; i < maxUsersCount; i++){
					if (threads[i] != null){
						threads[i].output_stream.println("#status " + name + ": " + sentence);
					}
				}
			}
			for (int i = 0; i <maxUsersCount; i++){
				if (threads[i] != null && threads[i] != this){
					threads[i].output_stream.println("#Leave " + name.split(" ")[1]);
				}
			}
